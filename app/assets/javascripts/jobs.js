...
var Jobs = {
  ...
  event: function(){
    ...
    this.$el.find('.gn-radio-button').bind('click', function(){
      var $child = $(this).find('input[type=radio][name="job[material]"]');
      self.$el.find('div[class*=radio-checked]').removeClass('radio-checked');
      $(this).addClass('radio-checked');
      $child.attr('checked', true); 
    });

    $('.gn-checkbox > input[type=checkbox]').bind('click', function(e){
      if($(this).is(':checked'))
    ...
    this.$el.find('div.toggle-btn').bind('click', function(e){
      self.$el.find('div[class*=btn-selected]').removeClass('btn-selected');
      self.$el.find(this).addClass('btn-selected');
    });
    ...
    this.$el.find('section#gn-sub-form-description .show').bind('click', function(e){
      self.$el.find('#job_notes').toggle('slow');
    });

    this.$el.find('#job_job_category_id').bind('change', function(e){
      self.$el.find('#gn-change-category').addClass('active-category');     
    });

    this.$el.find('#gn-change-category').bind('click', function(e){      
      if($(this).hasClass('active-category')){
        if(self.$el.find( "#job_job_category_id option:selected" ).val()=='1'){
          self.$el.find('#gn-form-job-business').hide();
          self.$el.find('#gn-form-job-home').show();
        }
        else{
          self.$el.find('#gn-form-job-home').hide();
          self.$el.find('#gn-form-job-business').show();        
        }
      }
      self.$el.find(this).removeClass('active-category');  
    });
    ...
    this.$el.find('.gn-job-time select').bind('change', function(){
      ...
      /*var is_empty = false;
      self.$el.find('.gn-job-time select').each(function(index, element){
         if($(element).val()=="" || self.$el.find('#job_when_date').val()=="" ){
          is_empty = true;
          return false;
        }
      });
